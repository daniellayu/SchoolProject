...
class MessagesDb_Students(object):
    def __init__(self, tablename="MessagesDb_S", messageId="messageId", from_id="from_id", to_id="to_id",
                 date="date", text="text"):
    ...
    def create_table(self):
        try:
            ...
            connection.commit()
            connection.close()
            print("succeed to create table MessagesDb_S")
            return True
        except:
            print("failed to create table MessagesDb_S")
            return False
    ...
    def get_all_messages_for_student(self, to_id):
        try:
            connection = sqlite3.connect("database.db")
            cursor = connection.cursor()
